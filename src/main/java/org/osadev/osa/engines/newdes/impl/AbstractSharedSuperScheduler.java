...
public abstract class AbstractSharedSuperScheduler<U extends Comparable<U>> implements SharedSuperSchedulerItf {
	...
	public void deployAndBind(Component superSched, Component rootComponent)
		...
		logger_.debug("Deploy and Bind: supersched={}, rootComp={}",superSched,rootComponent);
		...
		if ((superSched != null) && (self_ == null)) {
			...
			InterfacesEnum.OSA_SUPER_SCHEDULER_CONTROL.
					getInterface(superSched, "Superscheduler deploy: " +
					"superSched parameter is not a super scheduler ("+superSched+").");
		...
		for (Component comp: subComponents) {
			...
			try {
				simC = (EventSimulationControllerAPI<ModelingTimeAPI<Long>>)
						InterfacesEnum.OSA_SIMULATION_CONTROLLER.getInterface(comp);
			...
			if (simC != null) {
				...
				bc = (BindingController)InterfacesEnum.FC_BINDING_CONTROLLER.
						getInterface(comp,"Superscheduler deploy: Can't find BC on component "+comp);
				
				
				superSchedItf = (EventSuperSchedulerItf<ModelingTimeAPI<Long>>)
						InterfacesEnum.OSA_SUPER_SCHEDULER_SRV_ITF.
						getInterface(self_,"Superscheduler deploy: Can't find superschedulersvc"+ 
				...
				try {	
					...
					throw new OsaRuntimeException("Superscheduler deploy: Can't bind superscheduler client itf to superschedulersvc interface of super scheduler: Comp =" + comp,e);
