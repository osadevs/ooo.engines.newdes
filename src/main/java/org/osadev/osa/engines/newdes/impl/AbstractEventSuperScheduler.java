...
extends AbstractSharedSuperScheduler<U> implements EventSuperSchedulerItf<U>, SimulationControlAPI {
	...
	public AbstractEventSuperScheduler(final SimulationLogger<U> logger, final TimeFactoryItf<U> factory) {
		super(logger);
	...
	/*
	 * (non-Javadoc)
	 * @see org.osadev.osa.simapis.newdes.simulation.EventSuperSchedulerItf#waitUntil(org.osadev.osa.simapis.newdes.ModelingTime, org.osadev.osa.simapis.newdes.simulation.EventSimulationControllerAPI)
	 */
	public void waitUntil(ModelingTimeAPI<U> time, EventSimulationControllerAPI<U> callBack) {
		// Time value is useless because simulation controllers are supposed to be comparable
		// based on their next time of scheduling. However, assuming the sorting is to occur
		// at the time of insertion (which is not always true, but safest to consider)
	...
	/*
	 * (non-Javadoc)
	 * @see org.osadev.osa.simapis.newdes.simulation.SimulationControlAPI#startSimulation()
	 */
	public void startSimulation() {
		for (Component simC: this.boundComponents_) {
			
			@SuppressWarnings("unchecked")
			EventSimulationControllerAPI<U> esc = (EventSimulationControllerAPI<U>)
					InterfacesEnum.OSA_SIMULATION_CONTROLLER.getInterface(simC, "startSimulation");
			esc.init();
