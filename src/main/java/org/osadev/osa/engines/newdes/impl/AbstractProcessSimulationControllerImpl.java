...
extends AbstractEventSimulationControllerImpl<U> 
implements  ProcessSimulationControllerAPI<U>, ProcessModelingAPI<U>, Controller {

	@SuppressWarnings("rawtypes")
	private static  final  SimulationLogger LOGGER = new SimulationLogger(AbstractProcessSimulationControllerImpl.class);
	...
	private Multimap<String,WaitRequest<U>> condMap_ =  LinkedListMultimap.<String, WaitRequest<U>>create();
	
	private List<WaitRequest<U>> readyList_ = new LinkedList<WaitRequest<U>>();
	
	protected AbstractProcessSimulationControllerImpl(TimeFactoryItf<U> timeFactory, EventFactoryItf<U> eventFactory) {
	...
	/*
	 * (non-Javadoc)
	 * @see org.osadev.osa.simapis.newdes.ProcessModelingAPI#waitForDelay(org.osadev.osa.simapis.newdes.ModelingTime)
	 */
	public void waitForDelay(ModelingTimeAPI<U> delay) {
		WaitRequest<U> wr = new WaitRequest<U>();
		wr.setTimedOut();
		wr.setCondition(null);
		AbstractEvent<U> event = eventFactory_.create("wakeUpThread", new Object [] { wr } , this.getSimulationTime().getDelayed(delay.get()), this);
	...
	/*
	 * (non-Javadoc)
	 * @see org.osadev.osa.simapis.newdes.ProcessModelingAPI#waitOnConditionForDelay(java.lang.String, org.osadev.osa.simapis.newdes.ModelingTime)
	 */
	public String waitOnConditionForDelay(String condition, ModelingTimeAPI<U> delay)
	...
	public boolean releaseOneOnCondition(String condition, String param) {
		...
		return this.psuperSchedInterface_.iterateReleaseOneOnCondition(condition, param);
	}
	...
	public int releaseAllOnCondition(final String condition, final String param) {
		return this.psuperSchedInterface_.iterateReleaseAllOnCondition(condition, param);
	}
	...
	throws IllegalEventMethodException,	UnknownEventMethodException {
		...
		getLogger().debug("ProcessController::schedEventMyself(meth={},params={},time={})",
	...
	public void startProcess(final Object obj) {
		...
		t.start();
		this.psuperSchedInterface_.parkSchedulerThread("startProcess complete.");
	}

	
	/*
	 * (non-Javadoc)
	 * @see org.osadev.osa.simapis.newdes.ProcessModelingAPI#scheduleProcessMyself(java.lang.String, java.lang.Object[], org.osadev.osa.simapis.newdes.ModelingTime)
	 */
	public long scheduleProcessMyself(String methodName, Object[] parameters,
		...
		getLogger().debug("startProcessMyself(meth={},params={},time={})",
				methodName,parameters,time);
		// Scheduling a process consists in scheduling a control event that will create the process
		// which will in turn execute the business event.
		AbstractEvent<U> busEvent = eventFactory_.create(methodName, parameters, time, content_);
		AbstractEvent<U> ctlEvent = eventFactory_.create("startProcess", new Object[] {(Object)busEvent}, time, this);
	...
	/*
	 * (non-Javadoc)
	 * @see org.osadev.osa.simapis.newdes.simulation.ProcessSimulationControllerAPI#resumeReady()
	 */
	public boolean resumeReady() {
		...
		if (readyList_.size() > 0) {
			...
			getLogger().debug("Found this request in list: {} (permits={}).",wr,wr.availablePermits());
			wr.release();
			return true;
		}
		return false;
	...
	public boolean resumeNext(ModelingTimeAPI<U> currentTime)
		...
		if (super.resumeNext(currentTime)) {
			this.psuperSchedInterface_.parkSchedulerThread("resumeNext() woke succeeded to wake up a thread");
			return true;
		}
		return false;
	}
	...
	public void init() {
		...
		superSchedInterface_ = psuperSchedInterface_ = (ProcessSuperSchedulerItf<U>) 
				InterfacesEnum.OSA_SUPER_SCHEDULER_CLI_ITF
				.getInterface(getComponent(), "init");

		localSchedInterface_ = (ProcessSimulationControllerAPI<U>) 
		...
		psuperSchedInterface_.waitUntil(nextScheduledEvent_,localSchedInterface_);
	}
